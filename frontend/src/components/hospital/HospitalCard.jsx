...
const HospitalCard = ({ hospital, isSelected, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      {/* Hospital Type & Departments Tags */}
      <div className="flex items-center gap-1.5 flex-wrap mt-2.5">
        ...
        {hospital.departments && hospital.departments.slice(0, 3).map((dept, i) => (
          <span
            key={i}
            className="text-[10px] font-semibold px-2 py-0.5 rounded-md bg-sky-50 text-sky-700 border border-sky-200/80"
          >
            {dept}
          </span>
        ))}
      ...
      {/* Key Facilities Row */}
      <div className="flex items-center gap-2 flex-wrap mt-2 text-[10px] font-bold text-slate-600">
        {hospital.ambulanceAvailable && (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200">
            🚑 Ambulance
          </span>
        )}
        {hospital.bloodBankAvailable && (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-rose-50 text-rose-700 border border-rose-200">
            🩸 Blood Bank
          </span>
        )}
        {hospital.pharmacyAvailable && (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200">
            💊 24/7 Pharmacy
          </span>
        )}
        {hospital.diagnosticAvailable && (
          <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-amber-50 text-amber-800 border border-amber-200">
            🔬 Diagnostics
          </span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-3 mt-3 border-t border-slate-100 text-xs flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          {hospital.phone && (
            <a
              ...
              className="p-1.5 rounded-lg text-slate-600 hover:text-sky-600 hover:bg-sky-50 transition-colors border border-slate-200"
              title="Call Hospital"
            >
          ...
          <button
            ...
            className="flex items-center gap-1 text-xs font-bold text-sky-700 bg-sky-100/80 hover:bg-sky-200 px-2.5 py-1 rounded-lg transition-colors border border-sky-200 cursor-pointer shadow-xs"
          >
            Doctors &amp; Details <ArrowRight className="w-3 h-3" />
          </button>
        </div>
      </div>
    </div>
